-- ============================================================================
-- FEBRERO TOSTADORES DE CAFE - SETUP COMPLETO (ONE SHOT, IDEMPOTENTE)
-- ============================================================================
-- Proposito:
--   Script unico para dejar una Supabase NUEVA 100% funcional para el POS
--   (Next.js + Supabase/Postgres) de "Febrero Tostadores de Cafe".
...
--        VALUES ('<UUID_DE_AUTH>', 'admin@febrero.cafe', 'Administrador',
...
CREATE TABLE IF NOT EXISTS store_settings (
  ...
  store_name TEXT DEFAULT 'Febrero Tostadores de Café',
...
INSERT INTO store_settings (store_name, estimated_delivery_time)
  SELECT 'Febrero Tostadores de Café', 30 WHERE NOT EXISTS (SELECT 1 FROM store_settings);
INSERT INTO app_settings (key, value) VALUES
  ('bar_config', '{"tables_count": 20, "areas": ["main", "terraza"], "default_area": "main"}'::jsonb),
  ('general', '{"store_name": "Febrero Tostadores de Café", "currency": "ARS", "timezone": "America/Argentina/Buenos_Aires"}'::jsonb)
...
-- Credenciales por defecto: admin@febrero.cafe / Admin123456!
...
DO $$
DECLARE
  v_admin_email TEXT := 'admin@febrero.cafe';
...
SELECT 'Febrero Tostadores de Café', 30
...
-- ===================== SEED FEBRERO: CATALOGO DE CAFETERIA (ejemplo) =====================
...
-- ============================================================
-- Insumos + Recetas (costo de producción por producto, estilo Fudo)
-- ============================================================
CREATE TABLE IF NOT EXISTS public.ingredients (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  name text NOT NULL,
  unit text NOT NULL DEFAULT 'kg',   -- 'kg' | 'l' | 'unidad' (unidad de compra)
  cost numeric NOT NULL DEFAULT 0,    -- costo por esa unidad
  is_active boolean NOT NULL DEFAULT true,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
CREATE TABLE IF NOT EXISTS public.product_ingredients (
  ...
  quantity numeric NOT NULL DEFAULT 0, -- g si unit=kg, ml si unit=l, unidades si unit=unidad
  created_at timestamptz DEFAULT now()
);
...
DROP POLICY IF EXISTS "staff_manage_ingredients" ON public.ingredients;
CREATE POLICY "staff_manage_ingredients" ON public.ingredients FOR ALL TO authenticated USING (public.is_active_staff()) WITH CHECK (public.is_active_staff());
DROP POLICY IF EXISTS "staff_manage_product_ingredients" ON public.product_ingredients;
CREATE POLICY "staff_manage_product_ingredients" ON public.product_ingredients FOR ALL TO authenticated USING (public.is_active_staff()) WITH CHECK (public.is_active_staff());
